...
/**
 * The 404, as a view rather than a route.
 *
 * Two routes render it and they wrap it differently, which is the whole reason
 * it lives here. `app/not-found.tsx` is what Next serves for a URL the app does
 * not have, and it sits at the root — outside the (site) group — so it has to
 * bring the site's chrome with it. `(site)/404` is the linkable one the footer
 * points at, and it is inside the group, so the chrome is already around it.
 *
 * Keeping one copy is not tidiness. The two would drift, and the one that
 * drifted would be the one nobody looks at until they have mistyped a URL.
 *
